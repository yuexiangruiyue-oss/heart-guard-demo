...
- 每一帧越界都存在可物理阻断的位置;本包用 1200 个自动化测试给出实测:**拦截率 100%(823/823)、无误伤(377/377 放行)、判定延迟 p50 ≈ 0.098ms**
...
| S6 已授权动作内的恶意语义 | **§5.2 承认的缺口** | 如实放行并当场声明 |

红队:`python tests/test_1000_attacks.py`
```
必须拦截 823 → 823(100.0%) | 必须放行 377 → 377(100.0%)
漏拦 0 · 误伤 0 · p50 0.098ms / p95 0.243ms
```

## 五、诚实边界(按可解程度排序)

1. **已授权动作内的恶意语义**(S6 所示)——唯一纯工程缺口,升级方案:参数语义审查 + 第二验证器两钥匙制。**最先修。**
